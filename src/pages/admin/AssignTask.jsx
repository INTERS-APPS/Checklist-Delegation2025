...
export default function AssignTask() {
  ...
  const frequencies = [
    ...
    { value: "schedule", label: "Schedule (One Time on Checklist)" },
  ...
  const fetchMasterSheetOptions = async () => {
    try {
      const masterSheetId = "1pso64b1nmDBPtq9V5Ay0L93smot03LKat_K0wka0XDY";
  ...
  const getLastTaskId = async (sheetName) => {
    try {
      const url = `https://docs.google.com/spreadsheets/d/1pso64b1nmDBPtq9V5Ay0L93smot03LKat_K0wka0XDY/gviz/tq?tqx=out:json&sheet=${encodeURIComponent(
        sheetName
      )}`;
  ...
  const fetchWorkingDays = async () => {
    try {
      const sheetId = "1pso64b1nmDBPtq9V5Ay0L93smot03LKat_K0wka0XDY";
  ...
  // Updated generateTasks function that handles the new "schedule" frequency
  const generateTasks = async () => {
    ...
    // For "schedule" frequency, we don't need to check working days - use the selected date as is
    if (formData.frequency === "schedule") {
      const selectedDateStr = formatDateToDDMMYYYY(date);
    ...
    // For all other frequencies, use the existing working days logic
  ...
  // Updated handleSubmit function with updated sheet selection logic
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // - "one-time" frequency → DELEGATION sheet
      // - "schedule" frequency → Checklist sheet (new)
