...
const CONFIG = {
  ...
  DRIVE_FOLDER_ID: "1bhts4O4GhsggMN68BWZ_vRqslIQl7w2Y",
...
function AccountDataPage() {
  ...
  const handleSubmit = async () => {
    ...
    const missingRemarks = selectedItemsArray.filter((id) => {
      ...
      return (
        (additionalStatus === "Not Required" || additionalStatus === "Pending") && (!remarks || remarks.trim() === "")
      )
    })
    ...
    try {
      ...
      const submissionData = await Promise.all(
        selectedItemsArray.map(async (id) => {
          const item = accountData.find((account) => account._id === id)
          const taskStatus = additionalData[id] || ""
          ...
          return {
            taskId: item["col1"],
            rowIndex: item._rowIndex,
            actualDate: taskStatus === "Pending" ? "" : todayFormatted, // Don't store actual date if status is Pending
            status: taskStatus,
      ...
      if (result.success) {
        ...
        const submittedItems = selectedItemsArray.map((id) => {
          ...
          return {
            ...item,
            col10: taskStatus === "Pending" ? "" : todayFormatted, // Don't store actual date if status is Pending
            col12: taskStatus, // Status
            col13: remarksData[id] || "", // Remarks
            col14: item.image ? (typeof item.image === "string" ? item.image : "Image uploaded") : "", // Image info
          }
        })
        ...
        // Add submitted items to history data (only if they have actual completion date)
        const completedItems = submittedItems.filter((item) => item.col10 !== "")
        setHistoryData((prev) => [...completedItems, ...prev])

        setSuccessMessage(`Successfully processed ${selectedItemsArray.length} task records! ${completedItems.length > 0 ? 'Completed tasks moved to history.' : ''}`)
        setSelectedItems(new Set())
        setAdditionalData({})
        setRemarksData({})
                      ...
                      filteredHistoryData.map((history) => (
                        <tr key={history._id} className="hover:bg-gray-50">
                          ...
                          <td className="px-6 py-4 whitespace-nowrap bg-blue-50">
                            <span
                              className={`inline-flex px-2 py-1 text-xs font-semibold rounded-full ${history["col12"] === "Completed"
                        ...
                        <tr key={account._id} className={`${isSelected ? "bg-purple-50" : ""} hover:bg-gray-50`}>
                          ...
                          <td className="px-6 py-4 whitespace-nowrap bg-orange-50">
                            <input
                              type="text"
                              placeholder={
                                additionalData[account._id] === "Not Required" ||
                                  additionalData[account._id] === "Pending"
                                  ? "Remarks required *"
                                  : "Enter remarks"
                              }
                              ...
                              className={`border rounded-md px-2 py-1 w-full disabled:bg-gray-100 disabled:cursor-not-allowed ${additionalData[account._id] === "Not Required" ||
                                additionalData[account._id] === "Pending"
